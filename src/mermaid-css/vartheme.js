...
function adjust(color, channels) {
  if (typeof color === "string") {
    if (khroma.isValid(color)) {
      let out = khroma.adjust(color, channels);
      return khroma.toHsla(out);
    }
    let [name, args] = funcArgs(color);
    switch (name) {
      case "hsla":
        return adjust({
          fn: name,
        ...
        break;
      default:
        throw new Error(`unknown color type "${name}": adjust("${color}",${JSON.stringify(channels)})`);
    }
  }
  for (let key in channels) {
    if (!(key in color)) {
      throw new Error(`color does not contain channel "${key}", cannot adjust`);
    }
    color[key] = `calc( ${color[key]} + ${channels[key]}${TYPE_SUFFIX[key]})`
  }
  return `${color.fn}(${[...color.channels].map((c) => {
    return color[c];
  }).join(", ")})`
}
...
function invert(color) {
  ...
  switch (color.fn) {
    case "hsla":
      color.h = `calc( ${color.h} + 180deg )`;
      color.l = `calc(( ${color.l} - 100% ) * -1 )`;
      break;
    default:
      throw new Error(`invert not implemented for type "${color.fn}"`);
  }
  return `${color.fn}(${[...color.channels].map((c) => {
    return color[c];
  }).join(", ")})`;
...
const mkBorder = (color) => {
  if (typeof color === "string") {
    if (khroma.isValid(color) && !color.startsWith("hsl")) {
      let hslaStr = khroma.toHsla(color) 
      return mkBorder(hslaStr);
    }
    let [name, args] = funcArgs(color);
    switch (name) {
      case "hsl":
  ...
  switch (color.fn) {
    case "hsla":
      let l = color.l;
      color.s = `calc( ${color.s} - 40% )`;
      color.l = `calc( ${color.l} + (10% * ((max( 1, ${cssAbs(l)} ) / (( ${l} ) - ( ${cssAbs(l)} - max( 1, ${cssAbs(l)} )))))))`;
      break;
    default:
      throw new Error(`mkborder not implemented for type "${color.fn}"`);
  }
  return `${color.fn}(${[...color.channels].map((c) => {
    return color[c];
  }).join(", ")})`;
